...
export function eqChainFork(a: ChainFork, b: ChainFork) {
    return (
        eqChainPoint(a.intersection, b.intersection) &&
...
export function eqChainForkHeaders(a: ChainForkHeaders, b: ChainForkHeaders) {
    return (
        eqChainPoint(a.intersection, b.intersection) &&
        a.fragment.length === b.fragment.length &&
        a.fragment.every((ahdr, i) =>
            uint8ArrayEq(ahdr.hash, b.fragment[i].hash),
        )
    );
}
...
export class VolatileDb {
    ...
    async resolvePoint(point: RealPoint): Promise<MultiEraHeader | undefined> {
        ...
        return MultiEraHeader.fromCbor(new Uint8Array(await readFile(hdrPath)));
    }

    async extendMain(extension: MultiEraHeader): Promise<void> {
        const pt = pointFromHeader(extension);
        if (!uint8ArrayEq(this.tip.blockHeader.hash, extension.prevHash)) {
        ...
        if (this.main.length > this.chainDb.cfg.k) {
            const immutable = this.main.splice(
                0,
                this.main.length - this.chainDb.cfg.k,
            );
    ...
    async garbageCollection(): Promise<void> {
        ...
        const blockPaths = this.immutable.map(
            (pnt) =>
                `${this.path}/blocks/${toHex(pnt.blockHeader.hash)}-${pnt.blockHeader.slotNumber}`,
        );

        const immutablePath = `${this.chainDb.immutableDb.path}/${this.chainDb.immutableDb.chunks}.chunk`;
        logger.info(
            "moving",
            this.immutable.length,
            "blocks to file",
            immutablePath,
        );
        ...
        const oldOrphans = this.orphans.filter(
            (pnt) => pnt.blockHeader.slotNumber < anchor,
        );
        ...
        await Promise.all(
            blockPaths
                .concat(
                    // headerPaths (immutable)
                    this.immutable.map(
                        (pnt) =>
                            `${this.path}/headers/${toHex(pnt.blockHeader.hash)}-${pnt.blockHeader.slotNumber}`,
                    ),
                )
                .concat(
                    oldOrphans.map(
    ...
    getDistanceFromTipSync(point: RealPoint): number | undefined {
        for (let distance = 0; distance < this.main.length; distance++) {
            if (eqChainPoint(this.main[this.main.length - distance - 1], point))
                return distance;
        }
        return undefined;
    }
