...
export function isIVolatileChain(thing: any): thing is IVolatileChain {
    return (
        ...
        Array.isArray(thing.extension) &&
        thing.extension.every(isRealPoint)
    );
}

export class VolatileChain implements IVolatileChain {
    ...
    constructor(chain: IVolatileChain) {
        ...
        Object.defineProperties(this, {
            ...
            [Symbol.iterator]: {
                value: function* () {
                    yield self.anchor;
                    for (let i = 0; i < self.extension.length; )
                        yield self.extension[i++];
                },
                writable: false,
                enumerable: false,
                configurable: false,
