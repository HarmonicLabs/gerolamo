import { blake2b_224, verifyEd25519Signature_sync, VrfProof10 } from "@harmoniclabs/crypto";
...
import { Ord, toHex, uint8ArrayEq, writeBigUInt64BE } from "@harmoniclabs/uint8array-utils";
import { VrfCert } from "../../../../lib/ledgerExtension/common/vrfCert";
import { Rational } from "@harmoniclabs/cardano-ledger-ts";
...
```
where FixedDecimal is a wrapper of `DBig` and `DBig` wraps `IBig` and so on.
`34` is the precision of the number.

so just using bigint should be fine.
*/
const CERTIFIED_NATURAL_MAX = BigInt(
    "1157920892373161954235709850086879078532699846656405640394575840079131296399360000000000000000000000000000000000"
);

export async function assertHeader(
    ...
    const multi = MultiEraHeader.fromCbor( headerBytes );
    const header = multi.header as BabbageHeader;
    const absoluteSlot = header.slotNo;
    const issuer = header.issuerVkey;
    const poolId = blake2b_224( issuer );

    const declaredVrfKey = header.vrfVkey;
    const lStatePoolInfos = (await ledgerState.getPool( absoluteSlot, poolId ));
    if( !lStatePoolInfos ) throw new Error( `Unknown pool: ${toHex( poolId )}` );

    const registeredVrfKey = lStatePoolInfos.poolId;
    const leaderRelativeStake = lStatePoolInfos.stake / lStatePoolInfos.activeStake;

    if(!uint8ArrayEq(
        declaredVrfKey,
        registeredVrfKey
    )) throw new Error(
        "declared leader's VRF credentials differs from those registered in the ledger"
    );

    assertVrfProof( absoluteSlot, epochNonce, header.leaderVrfOutput, declaredVrfKey, header.vrfResult );

    await Promise.all([
        assertLeaderStake( activeSlotCoeff, leaderRelativeStake, header.leaderVrfOutput ),
        assertOpCert( header.operationalCert, issuer, ledgerState.latestOpCertSequenceNumber( poolId ) ),
        assertKesSignature( absoluteSlot, header.operationalCert.operationalCertKesPeriod, headerBytes, header.operationalCert.operationalCertHotVkey, header.bodySignature, ledgerState.maxKesEvolutions() )
    ]);
}

async function assertVrfProof(
    absoluteSlot: bigint,
    epochNonce: Uint8Array,
    vrfOutput: Uint8Array,
    poolVrfKey: Uint8Array,
    vrfResult: VrfCert
): Promise<void>
{
    const vrfInput = getVrfInput( absoluteSlot, epochNonce );
    const declaredProofHash = vrfResult[0];
    const blockProofBytes = vrfResult[1];
    const vrfProof = VrfProof10.fromBytes( blockProofBytes );

    if(!vrfProof.verify( poolVrfKey, vrfInput )) throw new Error( "Invalid VRF proof" );

    const proofHash = vrfProof.toHash();

    if(!uint8ArrayEq( declaredProofHash, proofHash ))
        throw new Error("Mismatch between the declared VRF proof hash in block");

    const calculatedLeaderVrfOutput = deriveVrfOutput( proofHash, VrfOutputDerivation.Leader );

    if(!uint8ArrayEq( calculatedLeaderVrfOutput, vrfOutput ))
        throw new Error("Mismatch between the declared VRF output in block");
}
...
{
    const result = new Uint8Array( 8 + epochNonce.length );
    writeBigUInt64BE( result, absoluteSlot, 0 );
    result.set( epochNonce, 8 );
    return result;
}

async function assertLeaderStake(
    active_slot_coeff: bigint,
    leader_relative_stake: bigint,
    certified_leader_vrf: bigint
): Promise<void>
{
    const denominator = CERTIFIED_NATURAL_MAX - certified_leader_vrf; 
    const recip_q = CERTIFIED_NATURAL_MAX / denominator;
    const c = bigIntNaturalLog( 1n - active_slot_coeff );
    const x = -(leader_relative_stake * c);
    const ordering: Ord = bigIntExpCmp( x, 1000n, 3n, recip_q );

    if( ordering !== Ord.LT ) throw new Error( "Insuficient leader stake" );
}

function assertOpCert(
    opCert: any, //  OperationalCert,
    issuerPubKey: Uint8Array,
    latestSequenceNumber: bigint | undefined
): void
{
    const signature: Uint8Array = opCert.signature;
    const declaredSequenceNumber: bigint = opCert.sequenceNumber;

    if( typeof latestSequenceNumber !== "bigint" )
        throw new Error("Unknown pool op cert sequence number");

    if( declaredSequenceNumber < latestSequenceNumber )
        throw new Error("Operational certificate sequence number is too small");

    if( (declaredSequenceNumber - latestSequenceNumber) > 1n ) // must be equal or one greater
        throw new Error("Operational certificate sequence number is too far ahead");
    
    const opCertMessage = new Uint8Array( opCert.hotKey.length + 8 + 8 );
    opCertMessage.set( opCert.hotKey, 0 );
    writeBigUInt64BE( opCertMessage, opCert.sequenceNumber, opCert.hotKey.length );
    writeBigUInt64BE( opCertMessage, opCert.kesPeriod, opCert.hotKey.length + 8 );

    if(!verifyEd25519Signature_sync(
        signature,
        opCertMessage,
        issuerPubKey
    )) throw new Error("Invalid operational certificate signature");
}

async function assertKesSignature(): Promise<void>
{
