...
export function byronEbbConsDataToCborObj({
    epochId,
    diff,
}: IByronEbbConsData): CborArray {
    return new CborArray([
        new CborUInt(epochId),
        new CborArray([new CborUInt(diff)]),
    ]);
}
...
export class ByronEbbHeader implements IByronEbbHeader {
    ...
    static fromCborObj(
        cbor: CborObj,
        _originalBytes?: Uint8Array | undefined,
    ): ByronEbbHeader {
        if (!(cbor instanceof CborArray && cbor.array.length >= 5))
            throw new Error("invalid cbor for ByronEbbHeader");

        const [cborMagic, cborPrevHash, cborBodyProof, cborConsData, extra] =
            cbor.array;
        ...
        const hdr = new ByronEbbHeader({
            ...
            hash: blake2b_256(
                new Uint8Array([0x82, 0x00, ..._originalBytes]),
            ) as U8Arr32,
