        ...
        async fetch(req: Request): Promise<Response> {
            ...
            if (url.pathname.startsWith('/utxo/')) {
                ...
                if (!/^[0-9a-f]{64}(:\d+)?$/i.test(ref)) {
                    return new Response('Invalid format: /utxo/{64hex-txhash} or /utxo/{64hex-txhash}:{index}', { status: 400 });
                }
                ...
                if (parts.length === 1) {
                    ...
                    if (utxos.length === 0) {
                        return new Response('No UTXOs found for tx hash', { status: 404 });
                    }
                    logger.info(`Found ${utxos.length} UTXOs for tx ${txHash.slice(0,8)}...`);
                    responseBody = JSON.stringify(utxos.map((u: any) => u.tx_out));
                } else {
                    // specific utxo_ref
                    const idx = parseInt(parts[1], 10);
                    if (isNaN(idx) || idx < 0) {
                        return new Response('Invalid output index', { status: 400 });
                    }
