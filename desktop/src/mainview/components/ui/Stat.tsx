...
export const Stat: Component<StatProps> = (props) => {
  ...
  return (
    <div class={cn("flex flex-col items-center text-center gap-1", props.class)} title={props.title}>
      <span class="text-[11px] uppercase tracking-[0.08em] text-text-dim font-medium">{props.label}</span>
      <span
        class={cn(
      ...
      {props.sub && <span class="text-[11px] text-text-muted">{props.sub}</span>}
    </div>
  );
};
