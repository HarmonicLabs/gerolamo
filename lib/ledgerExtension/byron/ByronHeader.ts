...
export function byronTxProofFromCborObj(cbor: CborObj): IByronTxProof {
    if (!(cbor instanceof CborArray && cbor.array.length >= 3))
        throw new Error(
            "invalid cbor for ByronHeader::IByronBodyProof::IByronTxProof",
        );

    const [cN, cHash1, cHash2] = cbor.array;
    ...
    return [Number(cN.num), cHash1.buffer as U8Arr32, cHash2.buffer as U8Arr32];
}
...
export function byronSscProofToCborObj(sscProof: IByronSscProof): CborArray {
    ...
    return new CborArray([fst, ...rest.map((b) => new CborBytes(b))]);
}

export function byronSscProofFromCborObj(cbor: CborObj): IByronSscProof {
    if (!(cbor instanceof CborArray && cbor.array.length >= 2))
        throw new Error(
            "invalid cbor for ByronHeader::IByronBodyProof::IByronSscProof",
        );

    const [cIdx, cHash1, cHash2] = cbor.array;

    if (
        !(
            (cIdx instanceof CborUInt && cHash1 instanceof CborBytes)
    ...
    if (!(0 <= idx && idx <= 3))
        throw new Error(
            "invalid index for ByronHeader::IByronBodyProof::IByronSscProof",
        );
    ...
    if (!(cHash2 instanceof CborBytes))
        throw new Error(
            "invalid cbor for ByronHeader::IByronBodyProof::IByronSscProof",
        );
...
export function byronBodyProofFromCborObj(cbor: CborObj): IByronBodyProof {
    if (!(cbor instanceof CborArray && cbor.array.length >= 4))
        throw new Error("invalid cbor for ByronHeader::IByronBodyProof");

    const [cTxProof, cSscProof, cDlgProof, cUpdProof] = cbor.array;

    if (!(cDlgProof instanceof CborBytes && cUpdProof instanceof CborBytes))
        throw new Error("invalid cbor for ByronHeader::IByronBodyProof");
...
export function lwdlgFromCborObj(cbor: CborObj): Lwdlg {
    ...
    const [cEpochRange, cIssuer, cDelegate, cCert] = cbor.array;
...
export function dlgFromCborObj(cbor: CborObj): Dlg {
    ...
    const [cEpoch, cIssuer, cDelegate, cCert] = cbor.array;
...
export type IByronBlockSig = [0, Uint8Array] | [1, LwdlgSig] | [2, DlgSig];

export function byronBlockSigToCborObj(bSig: IByronBlockSig): CborArray {
    switch (bSig[0]) {
        case 0: {
            return new CborArray([new CborUInt(0), new CborBytes(bSig[1])]);
            break;
        }
        case 1: {
            return new CborArray([
        ...
        default:
            throw new Error("unrecognized 'IByronBlockSig'");
    }
}
...
export function byronBlockSigFromCborObj(cbor: CborObj): IByronBlockSig {
    ...
    switch (idx) {
        ...
        default:
            throw new Error("unrecognized 'IByronBlockSig' index from cbor");
    }
...
export function byronConsDataFromCborObj(cbor: CborObj): IByronConsData {
    ...
    const [cSlotid, cPubkey, cDiff, cBlockSig] = cbor.array;
...
export function byronHeaderExtraFromCborObj(cbor: CborObj): IByronHeaderExtra {
    ...
    const [cVer, cSoftVer, attrs, cExtraProof] = cbor.array;
    ...
    return {
        version: [Number(verArr[0]), Number(verArr[1]), Number(verArr[2])],
        softwareVersion: [
...
export interface IByronHeader extends IHeader {
...
export class ByronHeader implements IByronHeader {
    ...
    static fromCborObj(
        ...
        if (!(cbor instanceof CborArray && cbor.array.length >= 5))
        ...
        _originalBytes =
            _originalBytes instanceof Uint8Array
                ? _originalBytes
        ...
        const hdr = new ByronHeader({
            ...
            hash: blake2b_256(
                new Uint8Array([0x82, 0x01, ..._originalBytes]),
            ) as U8Arr32,
            prevHash: cborPrevHash.buffer as U8Arr32,
            slotNo:
                consensusData.slotid.epoch * BigInt(21600) +
                consensusData.slotid.slot,
            isEBB: false,
