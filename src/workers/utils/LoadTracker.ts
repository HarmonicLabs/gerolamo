...
export class LoadTracker {
    ...
    constructor(config: Partial<LoadTrackerConfig> = defaultConfig) {
        let { maxAge, joinTollerance } = config;
        maxAge = Number.isSafeInteger(maxAge) ? maxAge : defaultConfig.maxAge;
        joinTollerance = Number.isSafeInteger(joinTollerance)
            ? joinTollerance
            : defaultConfig.joinTollerance;
    ...
    getLoad(): Load {
        ...
        const time = end - start;
        const workTime = this.timestamps.reduce(
            (acc, ts) => acc + getWorkTime(ts, end),
            0,
        );
        return {
            start: start,
    ...
    isTracking(): boolean {
        ...
        return lastEnd <= 0 || now - lastEnd <= this.joinTollerance;
    }
