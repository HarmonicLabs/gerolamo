...
export function getEraIdxAndHeaderBytes(multiEraHeaderBytes: Uint8Array): {
    ...
    if (!(lazy instanceof LazyCborArray && lazy.array.length === 2)) {
        logger.debug("not first array");
        logger.error(
            "unexpected roll forward data",
            lazy,
            toHex(multiEraHeaderBytes),
        );
    ...
    if (!(lazy instanceof LazyCborTag && lazy.data instanceof CborBytes)) {
