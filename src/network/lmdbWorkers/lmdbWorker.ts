...
const slotIndexDB = rootDB.openDB({ name: 'slot_to_hash', encoding: 'binary', keyEncoding: 'ordered-binary' });
...
async function handleRollback(rollbackPoint: ChainPoint) {
    if (!(rollbackPoint instanceof ChainPoint)) throw new Error("invalid rollback point");
