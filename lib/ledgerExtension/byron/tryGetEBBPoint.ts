...
export function tryGetEBBPoint(headerBytes: Uint8Array): RealPoint | undefined {
    ...
    if (!(lazyHead instanceof LazyCborArray && lazyHead.array.length === 5)) {
        return undefined;
    }
    const thirdLazy = Cbor.parseLazy(lazyHead.array[2]);

    if (!(thirdLazy instanceof CborBytes && thirdLazy.buffer.length === 32)) {
    ...
    if (!(epochIdCbor instanceof CborUInt)) return undefined;
