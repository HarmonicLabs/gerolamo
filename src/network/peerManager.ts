...
import { type Topology, adaptLegacyTopology, isLegacyTopology, isTopology } from "./topology";
...
export async function initPeerManager(config: GerolamoConfig): Promise<void> {
	...
	if (topology.bootstrapPeers) {
		for (const ap of topology.bootstrapPeers) {
			await addPeer(ap.address.toString(), ap.port, "bootstrap", allPeers, bootstrapPeers, hotPeers, config);
			await addPeer(ap.address.toString(), ap.port, "hot", allPeers, bootstrapPeers, hotPeers, config);
		};
	};
	...
	if (topology.localRoots) {
		for (const root of topology.localRoots) {
			for (const ap of root.accessPoints) {
				await addPeer(ap.address, ap.port, "hot", allPeers, bootstrapPeers, hotPeers, config);
			};
		};
	};
	...
	monitorInterval = setInterval(async () => {
		if (hotPeers.length === 0) {
			...
			await replenishPeers(topology, allPeers, bootstrapPeers, hotPeers, config);
		}
	}, 30000);
};

async function addPeer(host: string, port: number | bigint, category: string, allPeers: Map<string, PeerClient>, bootstrapPeers: PeerClient[], hotPeers: PeerClient[], config: GerolamoConfig): Promise<void> {
	try {
...
async function startSync(hotPeers: PeerClient[], config: GerolamoConfig): Promise<void> {
...
async function replenishPeers(topology: Topology, allPeers: Map<string, PeerClient>, bootstrapPeers: PeerClient[], hotPeers: PeerClient[], config: GerolamoConfig): Promise<void> {
	if (topology.bootstrapPeers) {
		for (const ap of topology.bootstrapPeers) {
			await addPeer(ap.address.toString(), ap.port, "hot", allPeers, bootstrapPeers, hotPeers, config);
		};
	...
	if (hotPeers.length > 0) {
		...
		somePeer.askForPeers().then(peers => {
			for (const p of peers) {
				addPeer(p.address.toString(), p.portNumber, "hot", allPeers, bootstrapPeers, hotPeers, config);
			}
		}).catch(err => logger.error(`Failed to get peers from ${somePeer.peerId}`, err));
	}
};
